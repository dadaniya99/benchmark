...
# ==================== API 调用 ====================
def call_api(base_url, api_key, model, messages, protocol='openai', timeout=300):
...
def run_benchmark(base_url, api_key, model, forced_protocol=None):
    global test_state
        ...
        for i, test in enumerate(TESTS):
            ...
            if test.get('multi_round'):
                ...
                for ri, rp in enumerate(test['rounds']):
                    ...
                    res = call_api(base_url, api_key, model, convo, protocol)
                ...
                res = call_api(base_url, api_key, model, msgs, protocol)
                # 403 错误自动停止
                ...
                res = call_api(base_url, api_key, model, [{'role': 'user', 'content': test['prompt']}], protocol)
                # 403 错误自动停止
                if not res['ok'] and '403' in str(res.get('error', '')):
                    score, note = 0, '失败(403权限错误，已自动停止)'
                    test_state['running'] = False
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        if self.path == '/api/start':
            ...
            thread = threading.Thread(target=run_benchmark, args=(body['base_url'], body['api_key'], body['model'], forced_protocol))
            thread.daemon = True
            thread.start()
            self.send_response(200)
            self.send_header('Content-Type', 'application/json')
...
const body={base_url:b,api_key:k,model:m};
...
.then(()=>{
document.getElementById('ct').textContent='正在停止...';
